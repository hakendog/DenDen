...
- Treat `setup brand apply` and `setup brand resume` as long-running commands: give each command at least 180 seconds and keep the user informed by yielding in bounded intervals no longer than 60 seconds.
- If either command times out, first check whether the same DenDen setup process is still running. While it remains alive, wait in bounded intervals; after it exits, run `setup brand resume`. Never rerun `setup brand apply`, remove the setup lock, or delete the pending checkpoint.
...
- For exposure or loss, ensure no appearance transfer is pending, run `setup rotate-plan`, obtain approval, then run `setup rotate --approved-digest <digest>`. Scan all retained phones again. Missing `brand-config.json` in an otherwise valid legacy sender setup is repairable: `rotate` creates a new appearance-management configuration after approval. Do not create it manually or stop for absence alone; an existing invalid or mismatched file still requires repair before rotation.
